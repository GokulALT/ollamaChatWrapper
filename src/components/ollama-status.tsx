...
import { Activity, CheckCircle2, AlertCircle, WifiOff, Loader2 } from 'lucide-react';
...
export function OllamaStatus() {
  ...
  useEffect(() => {
    const fetchStatus = async () => {
      ...
      try {
        ...
        setStatus({ online: false, message: 'Failed to fetch status from API.' });
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="p-2 space-y-2 text-sm group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:flex-col group-data-[collapsible=icon]:items-center">
      ...
      <div className="pl-2 text-xs text-sidebar-foreground/80 group-data-[collapsible=icon]:hidden">
